...
if __name__ == "__main__":
    ...
    manager = Manager()
    q = Queue()
    lstm_q = Queue()  # Fila para os valores do LSTM que vão para o Kalman
    kalman_q = Queue()  # Fila para os valores do Kalman (se precisar usar depois)
    ...
    p4 = Process(target=run_animation_lstm.run_animation_lstm, args=(q, lstm_q))    
    p3 = Process(target=run_animation_kmn.run_animation_kmn, args=(lstm_q, kalman_q))
    
    for p in (reader, p1, p2, p3, p4):
        p.start()

    # ---- Tkinter: hot-key Ctrl+N -------------------------------------------
    ...
    root.geometry("250x120+100+100")          # janela mínima (pode ser minimizada)
