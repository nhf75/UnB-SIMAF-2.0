...
def carregar_modelos():
    ...
    pasta_modelos = 'src/Trabalho Holdout/Modelos/'
...
def carregar_scaler():
    try:
        with open('src/Trabalho Holdout/Modelos/scaler.pkl', 'rb') as f:
...
def run_animation_lstm(originalData):
    ...
    model = load_model(os.path.join('src/Trabalho Holdout/Modelos', modelo_atual[0]))
    ...
    def animate_lstm(i):
        ...
        if len(xOriginalData) >= n_timesteps:
            input_sensor = np.array([xOriginalData[-n_timesteps:], yOriginalData[-n_timesteps:], zOriginalData[-n_timesteps:]]).T
            input_sensor_normalized = scaler.transform(input_sensor)
            ...
            predicted_error = scaler.inverse_transform(predicted_error.reshape(1, -1))[0]
            corrected_values = np.array([x_val, y_val, z_val]) - (predicted_error * fator_correcao[0])
            # corrected_values = (predicted_error * fator_correcao[0])
